...
export const allPortfolioBoughtAssetsFromAPI = selector({
  ...
  get: async ({ get }) => {
    ...
    const portfolioAssetsMarketData = await getWatchlistedCoins("1", boughtPortfolioAssets.map((portfolioAsset: any) => portfolioAsset?.id).join(","));
    ...
    return boughtAssets.sort((item1: any, item2: any) => item1.quantityBought * item1.currentPrice < item2.quantityBought * item2.currentPrice);
  },
});
